...
export const escapeRooms = pgTable(
  "escape_rooms",
  {
    id: uuid("id").primaryKey().defaultRandom(),
  ...
  (table) => [index("escape_rooms_location_idx").on(table.location)]
);
...
export const groupProposals = pgTable(
  ...
  (table) => [index("group_proposals_group_idx").on(table.groupId)]
);
...
export const proposalVotes = pgTable(
  ...
  (table) => [uniqueIndex("proposal_votes_unique_idx").on(table.proposalId, table.userId)]
);

// Match mode queue — users express interest, system auto-groups when threshold met
export const matchStatusEnum = pgEnum("match_status", ["waiting", "matched", "expired"]);
